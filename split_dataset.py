...
class DatasetSplitter:
    ...
    def split_samples(self, samples):
        ...
        print(f"\nDataset Split:")
        print(
            f"Training: {len(train_samples)} samples ({len(train_samples)/total_samples*100:.1f}%)"
        )
        print(
            f"Testing:  {len(test_samples)} samples ({len(test_samples)/total_samples*100:.1f}%)"
        )
        print(
            f"Train date range: {train_samples[0]['date']} to {train_samples[-1]['date']}"
        )
        print(
            f"Test date range:  {test_samples[0]['date']} to {test_samples[-1]['date']}"
        )
    ...
    def copy_files(self, samples, target_dir, split_name):
        ...
        manifest = {
            "split": split_name,
            "total_samples": len(samples),
            "date_range": {"start": samples[0]["date"], "end": samples[-1]["date"]},
            "label_distribution": {},
            "samples": [],
        }
    ...
    def create_split(self):
        ...
        print(
            f"Train samples: {summary['train']['samples']} ({summary['train']['percentage']:.1f}%)"
        )
        print(
            f"Test samples:  {summary['test']['samples']} ({summary['test']['percentage']:.1f}%)"
        )
