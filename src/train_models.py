...
def main():
    ...
    parser.add_argument(
        "--test-only", action="store_true", help="Test model creation without training"
    )
    parser.add_argument(
        "--compare", action="store_true", help="Compare both model architectures"
    )
...
def compare_models():
    ...
    try:
        ...
        print(
            f"  • Memory difference: ~{(base_params - hybrid_params) * 4 / 1e6:.1f} MB"
        )
