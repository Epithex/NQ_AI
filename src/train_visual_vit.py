...
class VisualViTTrainer:
    ...
    def create_model(self):
        ...
        # Create model
        self.model = create_proper_vit_model(
            image_size=224,
            num_classes=4,
            dropout=0.1
        )
    ...
    def setup_callbacks(self, epochs: int):
        ...
        tensorboard = tf.keras.callbacks.TensorBoard(
            log_dir=str(tensorboard_dir / f"visual_vit_{datetime.now().strftime('%Y%m%d_%H%M%S')}"),
            histogram_freq=1,
            write_graph=True,
            write_images=True
        )
    ...
    def evaluate_model(self):
        ...
        class_names = ['High Breakout', 'Low Breakdown', 'Range Expansion', 'Range Bound']
        ...
        if len(test_results) > 2:
            logging.info(f"  📊 Top-2 Accuracy: {test_results[2]:.4f} ({test_results[2]*100:.2f}%)")
        
        # Per-class performance
        logging.info("Per-class performance:")
        for i, class_name in enumerate(class_names):
            class_metrics = class_report[str(i)]
            logging.info(f"  {class_name}: P={class_metrics['precision']:.3f}, R={class_metrics['recall']:.3f}, F1={class_metrics['f1-score']:.3f}")
...
def main():
    ...
    if results['test_accuracy'] >= 0.70:
        ...
        print(f"❌ Target accuracy not reached. Got {results['test_accuracy']*100:.2f}%, need ≥70%")
