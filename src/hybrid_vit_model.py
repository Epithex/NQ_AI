...
class HybridViTModel:
    ...
    def create_image_encoder(self) -> keras.Model:
        ...
        self.logger.info(f"Image encoder created with {image_encoder.count_params()} parameters")
    ...
    def save_model_weights(self, weights_path: str = None):
        ...
        if weights_path is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            weights_path = f"{self.config['paths']['models']}/hybrid_vit_weights_{timestamp}.h5"
