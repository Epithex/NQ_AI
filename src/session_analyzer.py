...
class SessionAnalyzer:
    ...
    def analyze_session(self, session_data: pd.DataFrame, prev_high: float, prev_low: float) -> int:
        """
        ...
        """
        try:
            self.logger.info(f"Analyzing session with PDH={prev_high:.2f}, PDL={prev_low:.2f}")
    ...
    def is_rangebound(self, session_data: pd.DataFrame, prev_high: float, prev_low: float) -> bool:
        """
    ...
    def detect_simultaneous_touches(self, session_data: pd.DataFrame, prev_high: float, prev_low: float) -> bool:
        """
        ...
        """
        simultaneous = (session_data['High'] >= prev_high) & (session_data['Low'] <= prev_low)
    ...
    def track_level_touches(self, session_data: pd.DataFrame, prev_high: float, prev_low: float) -> int:
        """
        ...
        if high_touched and low_touched:
            ...
            if first_high_time < first_low_time:
                self.logger.info(f"High touched first at {first_high_time}, then low at {first_low_time}")
                return 3
            
            # Label 4: Red first, then green
            else:
                self.logger.info(f"Low touched first at {first_low_time}, then high at {first_high_time}")
                return 4
    ...
    def get_session_data(self, date: datetime, data: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(f"Session data: {len(session_data)} bars from {session_start} to {session_end}")
        return session_data
    ...
    def get_touch_details(self, session_data: pd.DataFrame, prev_high: float, prev_low: float) -> Dict:
        """
