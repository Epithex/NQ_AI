#!/usr/bin/env python3
"""
Pure Visual Dataset Creator for Multi-Instrument 4-Class Previous Day Levels Classification
Creates pure visual training datasets (ONLY 448x448 chart images) from DOW, NASDAQ, SP500 futures
Uses Excel data source with 4-class pattern analysis - NO numerical features
"""
...
# Import our pure visual analysis components
from pure_visual_pattern_analyzer import PureVisualPatternAnalyzer
from pure_visual_chart_generator import PureVisualChartGenerator
from excel_data_fetcher import ExcelDataFetcher


def process_sample_parallel(args: Tuple) -> Dict:
    """
    Process a single sample in parallel worker process for pure visual dataset.
    
    Args:
        args: Tuple containing (date, instrument, current_data, previous_data, config_path)
        
    Returns:
        Dictionary with sample information (ONLY visual features)
    """
    try:
        ...
        pattern_analyzer = PureVisualPatternAnalyzer(config_path)
        chart_generator = PureVisualChartGenerator(config_path)
        ...
        # Analyze 4-class daily pattern (same logic as hybrid)
        pattern = pattern_analyzer.analyze_daily_pattern(current_candle, previous_candle)
        
        # Generate 448x448 chart image with volume bars and reference lines
        chart_path = chart_generator.generate_pure_visual_chart_image(
            date, current_data, previous_candle, instrument
        )
        
        # Extract ONLY metadata for records (NO numerical features)
        metadata = pattern_analyzer.extract_metadata_features(current_candle, previous_candle)
        
        # Create pure visual sample record (ONLY visual + pattern)
        sample = {
            ...
            "metadata": metadata,  # Metadata only - NO numerical features for model
            "current_ohlc": {
            ...
            "pure_visual": True,  # Flag for pure visual approach
            "numerical_features": None,  # NO numerical features
            "success": True
        }
...
class PureVisualDatasetCreator:
    """Creates comprehensive pure visual datasets for multi-instrument training - NO numerical features."""

    def __init__(self, config_path: str = "config/config_pure_visual_daily.yaml", workers: int = None):
        """Initialize pure visual dataset creator."""
        ...
        self.pattern_analyzer = PureVisualPatternAnalyzer(config_path)
        self.chart_generator = PureVisualChartGenerator(config_path)
        ...
        self.dataset_manifest = {
            ...
            "features": {
                "visual": "448x448 charts with volume bars and previous day level reference lines",
                "numerical": "NONE - pure visual approach"
            }
        }
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(f"{log_dir}/pure_visual_dataset_creator.log"),
    ...
    def setup_directories(self):
        """Create necessary directories for pure visual dataset."""
        directories = [
    ...
    def create_pure_visual_samples(self, instrument: str, data: pd.DataFrame) -> List[Dict]:
        """
        Create pure visual classification samples for an instrument.
        ...
        Returns:
            List of pure visual sample dictionaries
        """
        # Choose parallel or sequential processing based on worker count
        if self.workers and self.workers > 1:
            return self.create_pure_visual_samples_parallel(instrument, data)
        else:
            return self.create_pure_visual_samples_sequential(instrument, data)

    def create_pure_visual_samples_sequential(self, instrument: str, data: pd.DataFrame) -> List[Dict]:
        """
        Create pure visual samples sequentially.
        ...
        for i in range(1, len(data)):
            try:
                ...
                # Analyze daily pattern (same logic as hybrid)
                pattern = self.pattern_analyzer.analyze_daily_pattern(current_candle, previous_candle)

                # Generate 448x448 chart with volume and previous day levels
                chart_path = self.chart_generator.generate_pure_visual_chart_image(
                    current_date, chart_data, previous_candle, instrument
                )

                # Extract ONLY metadata (NO numerical features)
                metadata = self.pattern_analyzer.extract_metadata_features(current_candle, previous_candle)

                # Create pure visual sample
                sample = {
                ...
                if len(samples) % 100 == 0:
                    self.logger.info(f"Created {len(samples)} pure visual samples for {instrument}")
    ...
    def create_pure_visual_samples_parallel(self, instrument: str, data: pd.DataFrame) -> List[Dict]:
        """
        Create pure visual samples using parallel processing.
    ...
    def create_multi_instrument_dataset(
        ...
        """
        Create pure visual dataset for multiple instruments with 4-class classification.
        ...
        try:
            ...
            self.logger.info(f"Creating pure visual dataset for {instruments} ({start_year}-{end_year})")
            self.logger.info(f"Model type: Pure visual (NO numerical features)")
            ...
            for instrument in instruments:
                ...
                # Create pure visual samples for this instrument
                samples = self.create_pure_visual_samples(instrument, data)
                ...
                self.logger.info(f"Completed {instrument}: {len(samples)} pure visual samples")
            ...
            result = {
                ...
                "model_type": "pure_visual_4class",
                "features": {
                    "visual": "448x448 charts with volume and previous day levels",
                    "numerical": "NONE - pure visual approach"
                }
            }
            
            self.logger.info(f"Pure visual dataset creation completed successfully: {total_samples} total samples")
            return result
            
        except Exception as e:
            self.logger.error(f"Pure visual dataset creation failed: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    def get_manifest_path(self) -> str:
        ...
        return os.path.join(
            self.config["paths"]["metadata"],
            f"pure_visual_dataset_manifest_{timestamp}.json"
        )
    ...
    def save_dataset_manifest(self):
        ...
        self.logger.info(f"Pure visual dataset manifest saved: {manifest_path}")

    def create_dataset_summary(self, instruments: List[str], total_samples: int) -> Dict:
        """Create comprehensive pure visual dataset summary."""
        summary = {
            "dataset_info": {
                "name": "Pure visual daily dataset",
                "model_type": "pure_visual_only",
                "total_samples": total_samples,
            ...
            "features": {
                "visual": {
                    "type": "448x448 daily candlestick charts with volume bars",
                    "reference_lines": "Previous day high (green) and low (red) levels",
                    "image_size": "448x448 pixels for high-resolution ViT",
                    "format": "PNG",
                    "volume_bars": "ENABLED for richer visual information"
                },
                "numerical": {
                    "count": 0,
                    "features": [],
                    "description": "NONE - pure visual approach only"
                }
            },
            "configuration": {
                "chart_bars": self.config["data"]["bars_per_chart"],
                "classification_classes": 4,
                "pure_visual_model": True,
                "numerical_features": False,
...
def main():
    """Test the pure visual dataset creator."""
    print("🚀 NQ_AI Pure Visual Dataset Creator")
    ...
    try:
        ...
        print("🏗️  Initializing pure visual dataset creator...")
        print(f"   📊 Model Type: Pure visual (NO numerical features)")
        print(f"   📈 Chart Features: 448x448 charts with volume bars and reference lines")
        ...
        if result["success"]:
            print("✅ Test pure visual dataset created successfully!")
        ...
        print("   - Pure visual: ONLY 448x448 chart images")
        print("   - NO numerical features extracted")
        print("   - Volume bars enabled for richer visual information")
        ...
        print("   - High resolution optimized for pure visual learning")

        print("\n✅ Pure visual dataset creator test completed successfully!")
        print("\n🚀 Ready for full pure visual dataset generation:")
        print("   python generate_pure_visual_daily_dataset.py --instruments ALL --parallel")
