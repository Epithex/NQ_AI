...
class ViTBaseTrainer:
    ...
    def setup_gpu(self):
        ...
        if gpus:
            try:
                ...
                self.logger.info(
                    f"Found {len(gpus)} GPU(s): {[gpu.name for gpu in gpus]}"
                )
    ...
    def load_datasets(self):
        ...
        try:
            ...
            self.logger.info(
                f"  Label distribution: {dataset_info['label_distribution']}"
            )
    ...
    def create_model(self):
        ...
        try:
            ...
            self.logger.info(
                f"  Model size: ~{total_params * 4 / 1e6:.1f} MB (float32)"
            )
    ...
    def setup_callbacks(self) -> list:
        ...
        checkpoint_path = (
            f"{self.paths['checkpoints']}/vit_base_best_{self.timestamp}.weights.h5"
        )
        ...
        try:
            ...
            if self.training_history:
                history_dict = {
                    ...
                    for k, values in self.training_history.history.items()
                }
            ...
            # Save model summary
            summary_path = (
                f"{self.paths['outputs']}/model_summary_vit_base_{self.timestamp}.txt"
            )
            ...
            config_path = (
                f"{self.paths['outputs']}/config_vit_base_{self.timestamp}.yaml"
            )
    ...
    def run_complete_training(self, epochs: int = None):
        ...
        try:
            ...
            self.logger.info("=" * 60)
            self.logger.info(
                "ViT-Base Hybrid Training Pipeline Completed Successfully!"
            )
            self.logger.info("=" * 60)
...
def main():
    ...
    parser.add_argument(
        "--test-model", action="store_true", help="Test model creation without training"
    )
