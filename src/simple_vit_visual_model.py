...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def __init__(
        self, embed_dim: int = 768, num_heads: int = 12, dropout: float = 0.1, **kwargs
    ):
        super().__init__(**kwargs)
        self.embed_dim = embed_dim
...
def compile_model(
    model: SimpleViT, learning_rate: float = 3e-4, class_weights: Optional[dict] = None
) -> SimpleViT:
    ...
    optimizer = tf.keras.optimizers.AdamW(
        learning_rate=learning_rate_schedule, weight_decay=0.01
    )
