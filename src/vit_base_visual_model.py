    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.vit_base = TFViTModel.from_pretrained(
            "google/vit-base-patch16-224-in21k", config=self.vit_config, from_tf=False
        )
        ...
        logging.info(
            f"Created PureVisualViTBase with {self.count_params():,} parameters"
        )
...
def create_pure_visual_model(
    num_classes: int = 4, image_size: int = 224, dropout_rate: float = 0.3
) -> PureVisualViTBase:
    """Create a pure visual ViT-Base model."""

    model = PureVisualViTBase(
        num_classes=num_classes, image_size=image_size, dropout_rate=dropout_rate
    )
...
def compile_model(
    ...
    optimizer = tf.keras.optimizers.AdamW(
        learning_rate=learning_rate_schedule, weight_decay=0.01, clipnorm=1.0
    )
