...
class MultiInstrumentDataFetcher:
    ...
    def fetch_instrument_data(
        ...
        try:
            self.logger.info(
                f"Fetching data for {instrument} from {start_date} to {end_date}"
            )
            ...
            if self.source == "stooq":
                try:
                    ...
                    if not data.empty:
                        self.logger.info(
                            f"Successfully fetched {len(data)} bars from stooq"
                        )
                except Exception as e:
                    self.logger.warning(
                        f"Stooq fetch failed: {str(e)}, trying fallback"
                    )
            ...
            if data is None or data.empty:
                ...
                if data.empty:
                    raise ValueError(
                        f"No data retrieved for {instrument} from either source"
                    )
            ...
            self.logger.info(
                f"Successfully fetched {len(data)} daily bars for {instrument}"
            )
            return data
    ...
    def get_previous_day_levels(
        self, current_date: pd.Timestamp, data: pd.DataFrame
    ) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            self.logger.debug(
                f"Previous day ({prev_date}): High={prev_high:.2f}, Low={prev_low:.2f}"
            )
    ...
    def get_data_for_chart(
        self, current_date: pd.Timestamp, data: pd.DataFrame, bars: int = 30
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if len(chart_data) < bars:
                self.logger.warning(
                    f"Only {len(chart_data)} bars available, requested {bars}"
                )
