...
class DatasetCreator:
    ...
    def generate_dataset(
        self, start_date: str, end_date: str, dataset_type: str = "training"
    ):
        """
        ...
        try:
            self.logger.info(
                f"Starting dataset generation from {start_date} to {end_date}"
            )
            ...
            while current_date <= end_dt:
                ...
                try:
                    ...
                    if sample:
                        samples_generated += 1
                        self.logger.info(
                            f"Generated sample {samples_generated} for {current_date.date()}"
                        )
            ...
            self.logger.info(
                f"Dataset generation complete: {samples_generated} samples created"
            )
    ...
    def process_trading_day(
        self, trading_date: datetime, all_data: pd.DataFrame
    ) -> Dict:
        """
        ...
        try:
            ...
            success_failure = self.session_analyzer.determine_success_failure(
                actual_label
            )
    ...
    def create_training_sample(
        ...
        sample = {
            ...
            "previous_levels": {"high": prev_high, "low": prev_low},
            "touch_details": touch_details,
            "metadata": {
    ...
    def create_dataset_summary(
        self, samples_generated: int, errors: List[str], dataset_type: str
    ):
        """
    ...
    def calculate_label_distribution(self) -> Dict[int, int]:
        ...
        label_files = [
            f for f in os.listdir(self.paths["labels"]) if f.endswith(".json")
        ]
