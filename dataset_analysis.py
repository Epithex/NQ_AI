...
def analyze_dataset():
    ...
    for label in sorted(label_counts.keys()):
        ...
        print(f"Label {label}: {count:3d} samples ({percentage:5.1f}%) - {label_names[label]}")
    ...
    print("-"*30)
    print(f"Chart Images: {len([f for f in os.listdir('data/images') if f.endswith('.png')])} PNG files")
